...
export function useClosestAdzan(jadwal: Jadwal[]) {
  const [closestUpcomingAdzan, setClosestUpcomingAdzan] =
    useState<Jadwal | null>(getClosestAdzan(jadwal));
  const [timeRemainingToClosestAdzan, setTimeRemainingToClosestAdzan] =
    useState(timeRemaining(closestUpcomingAdzan?.time ?? null));

  useEffect(() => {
    const interval = setInterval(() => {
      const closestAdzan = getClosestAdzan(jadwal);
      const timeToAdzan = timeRemaining(closestUpcomingAdzan?.time ?? null);
